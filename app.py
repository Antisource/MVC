...
st.markdown("This application is a dashboard that can be used to analyze motor vehicle collisions in New York City")

@st.cache_data(persist=True)
def load_data(nrows):
    data = pd.read_csv(DATA_URL, nrows=nrows, parse_dates={'date/time': ['CRASH_DATE', 'CRASH_TIME']})
...
st.pydeck_chart(pdk.Deck(
    ...
    layers=[
        pdk.Layer(
            "HexagonLayer",
            data=hourly_data[['longitude', 'latitude']],  # Corrected order of columns
